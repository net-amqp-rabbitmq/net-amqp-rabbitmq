...
ok $helper->exchange_declare(
  { exchange_type => "fanout", passive => 0, durable => 1, auto_delete => 1 } ),
  "exchange declare";
...
# Support a supplied ca cert
if ( $helper->{admin_api_cacert} ) {
  $ua->ssl_opts( SSL_ca_file => $helper->{admin_api_cacert} );
}
