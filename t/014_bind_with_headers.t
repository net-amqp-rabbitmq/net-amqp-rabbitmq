...
ok $helper->exchange_declare(
  { exchange_type => 'headers', auto_delete => 0 } ),
  "default exchange declare";
...
ok $helper->publish( "Routable", { headers => $headers } ),
...
ok !$helper->queue_bind( "", undef, undef, $headers ),
  "Binding to queue without a queue name";
ok !$helper->queue_bind( $queue, "", undef, $headers ),
